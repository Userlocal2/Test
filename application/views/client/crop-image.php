...
-->
				...
				<table class="result" style="float:right;margin:0;">
					<tr>
						...
						<td>
							<label>
								<?=$lang == '/ua' ? 'так' : 'yes' ;?>
								<input class="radd" style="margin:0 10px 0 2px;vertical-align:middle" type="radio" name="lak" value="1">
							</label>
							<label>
								<?=$lang == '/ua' ? 'ні' : 'no' ;?>
								<input class="radd" style="margin:0 10px 0 2px;vertical-align:middle" type="radio" name="lak" value="0" checked>
							</label>
						</td>
					</tr>
				</table>
  ...
  /*  $del_arr = array('0', '1','2','3');
    
    //unset($value[$key]);
    
 
    
    $keyy = array_search($textura, $del_arr);
...
//foreach(array_slice($textura, 4) as $k):
foreach ($textura as $k):
            

//print_r( array_slice  ($textura, 4));
...
//print_r(array_slice($textura, 1, 2, true ));
...
var Textura = {
	...
	all:$('.textura'),
	textura:null,
	data:{article:'',name:'<span style="color:#ff6000;"><b>'+ (window.lang == '/ua' ? 'виберіть текстуру' : 'Select the texture') +'</b></span>',lak:0,kley:0,eco:0,lat:0,uf:0,flip:'',filter:''},
	filter:$('a[data-filter]'),
	print:0,
	lak:0,
    kley:0,
	name:$('#your-textura'),
	load:'<div id="pre_post" style="position:absolute;zIndex:100000;background:#fff;opacity:0.5;top:0;bottom:0;right:0;left:0;"><table style="width:100%;height:100%;"><tr><td valign="center" align="center"><img src="<?echo base_url(); ?>/img/i/loading.gif"></td></tr></table></div>',
...
//          alert ($(this).attr('data-'+Textura.print)*1);
...
//console.log (this.textura.attr('data-lak'));
//console.log(this.data.lak);


//console.log ($( "input:radio[name=lak]:checked" ).val());   	 
             if((this.data.lak*1==0)){
...
//alert (this.data.lak);
       ...
       // 	$(':radio[name=lak]').attr('disabled','disabled') /*27.04*/
		//	$(':radio[name=lak]').eq(1).attr('checked','checked')
            
           // $(':radio[name=lak]').attr('disabled','')
           // alert (this.data.lak*1);
            
            //$(':radio[name=lak]').attr('disabled','disabled') /*мой вариант*/
            
     //alert ();
     //$(':radio[name=lak]').val("1");
    
    	//$(this).removeClass('checked')
        
        

$("input[name='lak'][value='0']").attr("checked", true);
            ...
            //$(':radio[name=lak]').enabled = false; 
            
		}else if ((this.data.lak*1!=0)){
		//	$(':radio[name=lak]').attr('disabled','')
           //$(':radio[name=lak]').enabled = true; 
           
           
          $(':radio[name=lak]').removeAttr('disabled');
	//	$(':radio[name=lak]').eq(1).attr('checked','checked')
        //alert ("helll");
        }
		
		//проверка на клей
		//if(!(this.data.kley*1)){ /*old*/
...
$(':radio[name=kley]').eq(1).attr('checked','checked')
	...
	summa:function(){
        ...
        //  if(($(':radio[name=lak]:checked').val()*1) > 0){
...
//console.log (this.textura.attr('data-lak'));
            ...
            l = sq*l;  /*строка расчета лака_old*/
           // l = sq*10;  /*строка расчета лака*/
  ...
  /*          if ($(this).attr('data-lak') == '')
            {

Textura.data.lak = '0';                                                
            }else {                
            
Textura.data.lak = $(':radio[name=lak]:checked').val();                
            }            
 console.log(Textura.data.lak);*/       
            
          //  Textura.data.lak  = $(':radio[name=lak]:checked').val();
           
            $("input[name=lak][value='0']").prop('checked', true);
...
if($.trim(phone.val()).length < 5 || $.trim(phone.val()).length > 19){
...
if($.trim(firstname.val()).length < 2 || $.trim(firstname.val()).length > 50){
				firstname.css({borderColor:'red',boxShadow:'0 0 5px #ff8c8c'});
				return false;
}
...
var lastnamee = $('input[name=name_last]');

if($.trim(lastnamee.val()).length < 3 || $.trim(lastnamee.val()).length > 19){
				lastnamee.css({borderColor:'red',boxShadow:'0 0 5px #ff8c8c'});
				return false;
			}

/*???*/

var adre_first = $('input[name=adre_first]');

if($.trim(adre_first.val()).length < 3 || $.trim(adre_first.val()).length > 35){
				adre_first.css({borderColor:'red',boxShadow:'0 0 5px #ff8c8c'});
				return false;
/*???*/			}



var adre_sec = $('input[name=adre_sec]');

if($.trim(adre_sec.val()).length < 3 || $.trim(adre_sec.val()).length > 35){
				adre_sec.css({borderColor:'red',boxShadow:'0 0 5px #ff8c8c'});
				return false;
			}
...
if($.trim(city.val()).length < 3 || $.trim(city.val()).length > 19){
				city.css({borderColor:'red',boxShadow:'0 0 5px #ff8c8c'});
				return false;
			}
...
if($.trim(country.val()).length < 3 || $.trim(country.val()).length > 19){
				country.css({borderColor:'red',boxShadow:'0 0 5px #ff8c8c'});
				return false;
			}
...
if($.trim(state.val()).length < 3 || $.trim(state.val()).length > 19){
				state.css({borderColor:'red',boxShadow:'0 0 5px #ff8c8c'});
				return false;
			}
...
if($.trim(zip.val()).length < 3 || $.trim(zip.val()).length > 19){
				zip.css({borderColor:'red',boxShadow:'0 0 5px #ff8c8c'});
				return false;
			}
...
result = (/^[0-9a-z_^\-\.]+@[0-9a-z_^\.\-]+\.[a-z]{2,6}$/i).test($.trim(email.val()));
console.log(result);
...
if($.trim(email.val()).length < 3 || $.trim(email.val()).length > 19){
				email.css({borderColor:'red',boxShadow:'0 0 5px #ff8c8c'});
				return false;
			}
            ...
            /*3.05*/
            $('#lak__').val($("input:radio[name=lak]:checked").val())
            /*3.05*/
